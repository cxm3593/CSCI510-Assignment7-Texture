...
function setUpTextures(){
	...
	// second texture
	oldWorldTexture = gl.createTexture();
	gl.bindTexture(gl.TEXTURE_2D, oldWorldTexture);

	var oldWorld_img = document.getElementById("oldWorld-texture");
	oldWorld_img.crossOrigin = "";

	oldWorld_img.onload = () => {
		gl.bindTexture(gl.TEXTURE_2D, oldWorldTexture);
		gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, oldWorld_img.width, oldWorld_img.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, oldWorld_img);
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
		gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
	}
...
function initProgram (vertexid, fragmentid) {
  ...
  program.isProc = gl.getUniformLocation(program, 'isProc');
